...
def stop_all(config):
    ...
    cmd = "ssh -o ConnectTimeout=6 %s@%s 'ps ax -o pid,user,command | grep edu.washington.escience.myria.daemon.MasterDaemon | grep %s | grep %s | grep -v grep | awk \"{print \\$1}\"'" % (username, hostname, username, deploy_name)
    ...
    for worker in workers:
        ...
        cmd = "ssh -o ConnectTimeout=6 %s@%s 'ps ax -o pid,user,command | grep edu.washington.escience.myria.parallel.Worker | grep %s | grep %s | grep -v grep | awk \"{print \\$1}\"'" % (username, hostname, username, deploy_name)
        try:
            pids = subprocess.check_output(cmd, shell=True).split('\n')
        except:
            continue
