...
public class SQLiteDeleteTest {
  ...
  public void test() throws Exception {
    ...
    final TupleSource deleteSource = new TupleSource(data);
    final DbInsert insert = new DbInsert(source, tuplesKey, SQLiteInfo.of(tempFile.getAbsolutePath()));
    final DbDelete delete = new DbDelete(deleteSource, tuplesKey, SQLiteInfo.of(tempFile.getAbsolutePath()));
    insert.open(null);
    while (!insert.eos()) {
      insert.nextReady();
    }
    ...
    final int result = statement.columnInt(0);
