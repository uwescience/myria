...
public class BenchmarkTest {
  ...
  public void init() {
    ...
    if (schema == null) {
      schema =
          Schema.of(ImmutableList.of(Type.INT_TYPE, Type.INT_TYPE), ImmutableList.of("i1", "i2"));
    ...
    try {
      String tempFilePath =
          Files.createTempDirectory(MyriaConstants.SYSTEM_NAME + "_systemtests").toString();
      ...
      jsonConnInfo =
          ConnectionInfo.toJson(
              MyriaConstants.STORAGE_SYSTEM_MYSQL,
              BENCHMARKTEST_HOSTNAME,
              workingDir,
              0,
              "myria1",
    ...
    for (ConnectionInfo conn : connections) {
      ...
      for (int i = 0; i < NUM_RUNS; i++) {
        DbQueryScan scan =
            new DbQueryScan(
                conn,
                "SELECT * FROM " + relationKey.toString(conn.getDbms()),
                Schema.of(ImmutableList.of(Type.LONG_TYPE), ImmutableList.of("count")));
        scan.open(null);
        int count = 0;
        TupleBatch tb = null;
        while (!scan.eos()) {
    ...
    for (ConnectionInfo conn : connections) {
      ...
      for (int i = 0; i < NUM_RUNS; i++) {
        DbQueryScan scan =
            new DbQueryScan(
                conn,
                "SELECT I1 FROM " + relationKey.toString(conn.getDbms()),
                Schema.of(ImmutableList.of(Type.LONG_TYPE), ImmutableList.of("count")));
        scan.open(null);
        int count = 0;
        TupleBatch tb = null;
    ...
    for (ConnectionInfo conn : connections) {
      ...
      for (int i = 0; i < NUM_RUNS; i++) {
        DbQueryScan scan =
            new DbQueryScan(
                conn,
                "SELECT * FROM " + relationKey.toString(conn.getDbms()) + " ORDER BY I1",
                Schema.of(ImmutableList.of(Type.LONG_TYPE), ImmutableList.of("count")));
        scan.open(null);
        int count = 0;
        TupleBatch tb = null;
      ...
      for (int i = 0; i < NUM_RUNS; i++) {
        DbQueryScan scan =
            new DbQueryScan(
                conn,
                "SELECT * FROM "
                    + relationKey.toString(conn.getDbms())
                    + " R1, "
                    + relationKey.toString(conn.getDbms())
                    + " R2 WHERE R1.I2 < 10000 AND R1.I1 = R2.I2",
                Schema.of(ImmutableList.of(Type.LONG_TYPE), ImmutableList.of("count")));
        scan.open(null);
        int count = 0;
        TupleBatch tb = null;
