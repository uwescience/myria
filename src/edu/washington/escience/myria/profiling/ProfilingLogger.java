...
public class ProfilingLogger {
  ...
  private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory
      .getLogger(ProfilingLogger.class);
  ...
  public ProfilingLogger(final ConnectionInfo connectionInfo) throws DbException {
    ...
    accessMethod.createTableIfNotExists(MyriaConstants.SENT_PROFILING_RELATION,
  ...
  protected void createSentIndex() throws DbException {
    ...
    List<IndexRef> index =
        ImmutableList.of(IndexRef.of(schema, "queryId"), IndexRef.of(schema, "subQueryId"),
            IndexRef.of(schema, "fragmentId"), IndexRef.of(schema, "destWorkerId"));
    try {
  ...
  protected void createResourceIndex() throws DbException {
    ...
    try {
      accessMethod
          .createIndexIfNotExists(MyriaConstants.RESOURCE_PROFILING_RELATION, schema, index);
    } catch (DbException e) {
  ...
  protected void createEventIndexes() throws DbException {
    ...
    List<IndexRef> rootOpsIndex =
        ImmutableList.of(IndexRef.of(schema, "queryId"), IndexRef.of(schema, "subQueryId"),
            IndexRef.of(schema, "fragmentId"), IndexRef.of(schema, "startTime"),
            IndexRef.of(schema, "endTime"));
    List<IndexRef> filterIndex =
        ImmutableList.of(IndexRef.of(schema, "queryId"), IndexRef.of(schema, "subQueryId"),
            IndexRef.of(schema, "fragmentId"), IndexRef.of(schema, "opId"),
            IndexRef.of(schema, "startTime"), IndexRef.of(schema, "endTime"));

    try {
      accessMethod.createIndexIfNotExists(MyriaConstants.EVENT_PROFILING_RELATION, schema,
          rootOpsIndex);
      accessMethod.createIndexIfNotExists(MyriaConstants.EVENT_PROFILING_RELATION, schema,
          filterIndex);
    } catch (DbException e) {
   ...
   * 
   * The time that we log is the relative time to the beginning of the query in nanoseconds
   * (workerStartTimeMillis). We assume that all workers get initialized at the same time so that
   * the time in ms recorded on the workers can be considered 0. The time in nanoseconds is only a
   * relative time with regard to some arbitrary beginning and it is different of different threads.
   * This means we have to calculate the difference on the thread and add it to the difference in ms
   * between the worker and the thread (startupTimeMillis).
   * 
  ...
  public long getTime(final Operator operator) {
    ...
    Preconditions.checkState(workerStartTimeMillis > 0,
        "Query initialization time has not been recorded.");
    ...
    Preconditions.checkState(startupTimeMillis >= 0,
        ...
        workerStartTimeMillis, threadStartTimeMillis);
  ...
  /**
   * Appends a single event appearing in an operator to a batch that is flushed either after a
   * certain number of events are in the batch or {@link #flushEventsBatch()} is called.
   * 
   ...
   */
  public synchronized void recordEvent(final Operator operator, final long numTuples,
      final long startTime) throws DbException {
  ...
  /**
   * Record that data was sent to a worker. The buffer is flushed at a particular number of tuples
   * or on a call to {@link #flush()}.
   * 
   * @param operator the operator where this record was logged
   ...
   * @throws DbException if insertion in the database fails
   */
  public synchronized void recordSent(final Operator operator, final int numTuples,
      final int destWorkerId) throws DbException {
    SubQueryId sq = operator.getSubQueryId();
  ...
  /**
   * Record a single resource stats. The buffer is flushed at a particular number of tuples or on a
   * call to {@link #flush()}.
   * 
  ...
  /**
   * Flush the profiling buffers. The buffer is flushed at a particular number of tuples or on a
   * call to {@link #flush()}.
   * 
   * @throws DbException if insertion in the database fails
   */
  ...
  private void flush(final RelationKey relationKey, final TupleBatch tupleBatch) throws DbException {
    ...
    LOGGER.info("Writing profiling data to {} took {} milliseconds.", relationKey,
