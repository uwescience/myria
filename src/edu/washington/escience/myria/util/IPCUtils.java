...
public final class IPCUtils {
   ...
   * shutdown TM.
   * */
  public static final TransportMessage CONTROL_SHUTDOWN = TransportMessage.newBuilder()
      .setType(TransportMessage.Type.CONTROL)
      .setControlMessage(ControlMessage.newBuilder().setType(ControlMessage.Type.SHUTDOWN)).build();

  /** Heartbeat message sent from a worker to tell the master that it is alive. */
  public static final TransportMessage CONTROL_WORKER_HEARTBEAT = TransportMessage.newBuilder()
      .setType(TransportMessage.Type.CONTROL)
      .setControlMessage(ControlMessage.newBuilder().setType(ControlMessage.Type.WORKER_HEARTBEAT))
      .build();
  ...
  public static TransportMessage addWorkerTM(final int workerId, final SocketInfo socketinfo) {
    return TransportMessage
        ...
        .setType(TransportMessage.Type.CONTROL)
        .setControlMessage(
            ControlMessage.newBuilder().setType(ControlMessage.Type.ADD_WORKER)
                .setWorkerId(workerId).setRemoteAddress(socketinfo.toProtobuf())).build();
  }
  ...
  public static TransportMessage queryReadyTM(final SubQueryId taskId) {
    return QUERY_TM_BUILDER.get()
        .setQueryMessage(queryMessageOf(taskId, QueryMessage.Type.QUERY_READY_TO_EXECUTE)).build();
  }
   ...
   */
  private static QueryMessage.Builder queryMessageOf(final SubQueryId taskId,
      final QueryMessage.Type type) {
    Objects.requireNonNull(taskId, "taskId");
    Objects.requireNonNull(type, "type");
    return QueryMessage.newBuilder().setQueryId(taskId.getQueryId())
        .setSubqueryId(taskId.getSubqueryId()).setType(type);
  }
   ...
   * */
  public static TransportMessage queryCompleteTM(final SubQueryId taskId,
      final ExecutionStatistics statistics) {
    return QUERY_TM_BUILDER
        .get()
        .setQueryMessage(
            queryMessageOf(taskId, QueryMessage.Type.QUERY_COMPLETE).setQueryReport(
                QueryReport.newBuilder().setSuccess(true)
                    .setExecutionStatistics(statistics.toProtobuf()))).build();
  }

  /**
   * Make sure a {@link Throwable} is serializable. If any of the {@Throwable}s in the
   * given err error hierarchy (The hierarchy formed by caused-by and suppressed) is not
   * serializable, it will be replaced by a {@link DbException}. The stack trace of the original
   * {@link Throwable} is kept.
   * 
  ...
  public static TransportMessage queryFailureTM(final SubQueryId taskId, final Throwable cause,
    ...
    return QUERY_TM_BUILDER
        ...
        .setQueryMessage(
            queryMessageOf(taskId, QueryMessage.Type.QUERY_COMPLETE).setQueryReport(
                QueryReport.newBuilder().setSuccess(false)
                    .setExecutionStatistics(statistics.toProtobuf())
                    .setCause(ByteString.copyFrom(inMemBuffer.toByteArray())))).build();
  }
  ...
  public static TransportMessage startQueryTM(final SubQueryId taskId) {
    return QUERY_TM_BUILDER.get()
        .setQueryMessage(queryMessageOf(taskId, QueryMessage.Type.QUERY_START)).build();
  }
   ...
   * */
  public static TransportMessage normalDataMessage(final List<? extends Column<?>> dataColumns,
      final int numTuples) {
    ...
    return DATA_TM_BUILDER
        .get()
        .setDataMessage(
            NORMAL_DATAMESSAGE_BUILDER.get().clearColumns()
                .addAllColumns(Arrays.asList(columnProtos)).setNumTuples(numTuples)).build();
  }
  ...
  public static TransportMessage normalDataMessage(final List<Column<?>> dataColumns,
    ...
    return DATA_TM_BUILDER
        ...
        .setDataMessage(
            NORMAL_DATAMESSAGE_BUILDER.get().clearColumns()
                .addAllColumns(Arrays.asList(columnProtos)).setNumTuples(validIndices.length()))
        .build();
  }
    ...
    return QUERY_TM_BUILDER
        ...
        .setQueryMessage(
            queryMessageOf(taskId, QueryMessage.Type.QUERY_DISTRIBUTE).setQuery(
                QueryProto.Query.newBuilder().setQuery(
  ...
  public static TransportMessage killQueryTM(final SubQueryId taskId) {
    return QUERY_TM_BUILDER.get()
        .setQueryMessage(queryMessageOf(taskId, QueryMessage.Type.QUERY_KILL)).build();
  }
  ...
  public static TransportMessage resourceReport(final List<ResourceStats> resourceUsage) {
    ...
    return TransportMessage.newBuilder().setType(TransportMessage.Type.CONTROL)
        .setControlMessage(ret.build()).build();
  }
}
