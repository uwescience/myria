...
public final class JsonAPIUtils {
   ...
   * */
  public static HttpURLConnection submitQuery(final String masterHostname, final int apiPort,
      final File queryFile) throws IOException {
   ...
   */
  private static URL getDatasetUrl(final String host, final int port, final String user,
      final String program, final String relation, final String format) throws IOException {
    return new URL(String.format(
        "http://%s:%d/dataset/user-%s/program-%s/relation-%s/data?format=%s", host, port, user,
   ...
   * @throws IOException if an error occurs
   */
  public static String download(final String host, final int port, final String user,
      final String program, final String relation, final String format) throws IOException {
   ...
   * @throws IOException if an error occurs
   */
  public static void replace(final String host, final int port, final String user,
      final String program, final String relation, final String dataset, final String format)
      throws IOException {
    URL url = getDatasetUrl(host, port, user, program, relation, format);
    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    conn.setDoOutput(true);
   ...
   * @throws IOException if IO errors
   * */
  public static HttpURLConnection ingestData(final String masterHostname, final int apiPort,
      final File queryFile) throws IOException {
   ...
   * @throws IOException if IO errors
   * */
  public static HttpURLConnection deleteDataset(final String masterHostname, final int apiPort,
      final String user, final String program, final String relation) throws IOException {
    String type = "application/json";
    URL u =
        new URL(String.format("http://%s:%d/dataset/user-%s/program-%s/relation-%s/",
            masterHostname, apiPort, user, program, relation));
    HttpURLConnection conn = (HttpURLConnection) u.openConnection();
