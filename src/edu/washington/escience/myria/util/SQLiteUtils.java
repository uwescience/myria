...
public final class SQLiteUtils {
  ...
  public static String typeToSQLiteType(final Type type) {
    switch (type) {
      ...
      case STRING_TYPE:
        return "TEXT";
      case DATETIME_TYPE:
        return "INTEGER"; // SQLLite does not support date type. use long to store the difference,
                          // measured in
  ...
  /**
   * Create a SQLite table. This method always creates an empty table, so if the table already
   * exists, all its contents will be removed.
   * 
  ...
  public static void createTable(final String dbFileAbsolutePath, final RelationKey relationKey,
      final String sqlSchemaString, final boolean replaceExisting, final boolean swipeData)
    ...
    try {
      ...
      if (statement.step()) {
        ...
        if (replaceExisting) {
          statement =
              sqliteConnection.prepare("drop table "
          ...
          statement =
              sqliteConnection.prepare("delete from "
                  + relationKey.toString(MyriaConstants.STORAGE_SYSTEM_SQLITE));
          statement.step();
          statement.reset();
          return;
      ...
      statement =
          sqliteConnection.prepare("create table "
  ...
  public static void deleteTable(final String dbFileAbsolutePath, final RelationKey relationKey)
    ...
    try {
      ...
      statement =
          sqliteConnection.prepare("drop table if exists "
              + relationKey.toString(MyriaConstants.STORAGE_SYSTEM_SQLITE));
      statement.step();
