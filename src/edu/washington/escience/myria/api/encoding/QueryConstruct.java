...
public class QueryConstruct {
  ...
  private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory
      .getLogger(QueryConstruct.class);
  ...
  public static Map<Integer, SubQueryPlan> instantiate(final List<PlanFragmentEncoding> fragments,
    ...
    for (PlanFragmentEncoding fragment : fragments) {
      RootOperator op =
          instantiateFragment(fragment, args, instantiatedFragments, op2OwnerFragmentMapping,
              allOperators);
   ...
   */
  public static void setQueryExecutionOptions(final Map<Integer, SubQueryPlan> plans,
      final FTMode ftMode, @Nonnull final Set<ProfilingMode> profilingMode) {
  ...
  private static boolean setOrVerifyFragmentWorkers(@Nonnull final PlanFragmentEncoding fragment,
    ...
    if (fragment.workers == null) {
      ...
      Preconditions.checkArgument(
          ...
          "During %s, cannot change workers for fragment %s from %s to %s", currentTask,
          fragment.fragmentIndex, fragment.workers, workers);
      return false;
    }
  }
   ...
   */
  private static void setAndVerifyScans(final List<PlanFragmentEncoding> fragments,
      final ConstructArgs args) throws CatalogException {
    ...
    for (PlanFragmentEncoding fragment : fragments) {
      for (OperatorEncoding<?> operator : fragment.operators) {
        ...
        Preconditions.checkArgument(scanWorkers != null, "Unable to find workers that store %s",
            scanRelation);
        /*
         * Note: the current assumption is that all the partitions need to be scanned. This will not
         * be true if we have data replication, or allow to scan only a subset of the partitions.
         * Revise if needed.
         */
  ...
  public static void sanityCheckEdges(final List<PlanFragmentEncoding> fragments) {
    ...
    Preconditions.checkArgument(consumedNotProduced.isEmpty(),
        "Missing producer(s) for consumer(s): %s", consumedNotProduced);
    Set<Integer> producedNotConsumed = Sets.difference(producerMap.keySet(), consumerMap.keySet());
    Preconditions.checkArgument(producedNotConsumed.isEmpty(),
        "Missing consumer(s) for producer(s): %s", producedNotConsumed);
    ...
    for (Integer opId : soleConsumer) {
      ...
      Preconditions.checkArgument(consumers.size() == 1,
          "Producer %s only supports a single consumer, not %s", opId, consumers.size());
    }
  }

  /**
   * Verify and propagate worker assignments of LocalMultiwayProducer/Consumer. Fragments containing
   * LocalMultiwayProducers/Consumers with the same operator ID need to be assigned to the same set
   * of workers.
   * 
   * @see #assignWorkersToFragments(List, ConstructArgs)
   * 
  ...
  private static void verifyAndPropagateLocalEdgeConstraints(
    ...
    Set<Integer> consumedNotProduced = Sets.difference(consumerMap.keySet(), producerMap.keySet());
    Preconditions.checkArgument(consumedNotProduced.isEmpty(),
        "Missing LocalMultiwayProducer(s) for consumer(s): %s", consumedNotProduced);
    Set<Integer> producedNotConsumed = Sets.difference(producerMap.keySet(), consumerMap.keySet());
    Preconditions.checkArgument(producedNotConsumed.isEmpty(),
        "Missing LocalMultiwayConsumer(s) for producer(s): %s", producedNotConsumed);
    ...
    for (PlanFragmentEncoding fragment : fragments) {
      for (OperatorEncoding<?> operator : fragment.operators) {
        if (operator instanceof CollectConsumerEncoding || operator instanceof SingletonEncoding
            ...
            || operator instanceof BinaryFileScanEncoding || operator instanceof FileScanEncoding
          ...
          if (fragment.workers == null) {
            ...
            Preconditions.checkArgument(fragment.workers.size() == 1,
                "Fragment %s has a singleton operator %s, but workers %s", fragment.fragmentIndex,
                operator.opId, fragment.workers);
          }
  ...
  private static void fillInRealOperatorAndWorkerIDs(final List<PlanFragmentEncoding> fragments) {
    ...
    /*
     * First pass: create a new ExchangePairID for each Consumer, and set it. Also track the workers
    ...
    /*
     * Second pass: set the ExchangePairIDs for each producer, also the workers for these and the
     * consumers.
     */
    for (PlanFragmentEncoding fragment : fragments) {
      for (OperatorEncoding<?> operator : fragment.operators) {
        if (operator instanceof AbstractConsumerEncoding<?>) {
          AbstractConsumerEncoding<?> consumer = (AbstractConsumerEncoding<?>) operator;
          consumer.setRealWorkerIds(ImmutableSet.copyOf(producerWorkerMap
              .get(consumer.argOperatorId)));
        } else if (operator instanceof AbstractProducerEncoding<?>) {
          ...
          idbController.setRealEosControllerOperatorID(consumerMap.get(idbController.opId)
              .iterator().next());
        }
      }
    }
   ...
   * <li>Edge constraints between fragments. E.g., a {@link LocalMultiwayProducerEncoding} must use
   * the same set of workers as its consumer.</li>
   * <li>Singleton constraints: Fragments with a {@link CollectConsumerEncoding} or a
   * {@link SingletonEncoding} must run on a single worker. If none is still set, choose an
   * arbitrary worker.</li>
   * <li>Unspecified: Any fragments that still have unspecified worker sets will use all workers in
   * the cluster.</li>
   * </ol>
   * 
   * @param fragments
   ...
   *         the catalog
   */
  private static void assignWorkersToFragments(final List<PlanFragmentEncoding> fragments,
      final ConstructArgs args) throws CatalogException {
    ...
    /*
     * 5. Again, verify and propagate worker assignments using LocalMultiwayProducer/Consumer
     * constraints.
     */
    verifyAndPropagateLocalEdgeConstraints(fragments);
   ...
   */
  private static RootOperator instantiateFragment(final PlanFragmentEncoding planFragment,
      final ConstructArgs args,
    ...
    for (OperatorEncoding<?> encoding : planFragment.operators) {
      ...
      op.setOpName(MoreObjects.firstNonNull(encoding.opName,
          "Operator" + String.valueOf(encoding.opId)));
      ...
      if (op instanceof RootOperator) {
        if (fragmentRoot != null) {
          throw new MyriaApiException(Status.BAD_REQUEST, "Multiple "
              + RootOperator.class.getSimpleName() + " detected in the fragment: "
              + fragmentRoot.getOpName() + ", and " + encoding.opId);
        }
   ...
   * in every relation written by a subquery. The query plan is basically
   * "SELECT RelationKey, COUNT(*)" -> Collect at master -> "SELECT RelationKey, SUM(counts)".
   * 
  ...
  public static SubQuery getRelationTupleUpdateSubQuery(
    ...
    Schema schema =
        Schema.ofFields("userName", Type.STRING_TYPE, "programName", Type.STRING_TYPE,
            "relationName", Type.STRING_TYPE, "tupleCount", Type.LONG_TYPE);

    String dbms = server.getDBMS();
    Preconditions.checkState(dbms != null,
        "Server must have a configured DBMS environment variable");

    /*
     * Worker plans: for each relation, create a {@link DbQueryScan} to get the count, an {@link
     * Apply} to add the {@link RelationKey}, then a {@link CollectProducer} to send the count to
     * the master.
     */
    ...
    for (RelationWriteMetadata meta : relationsWritten.values()) {
      ...
      for (Integer worker : workers) {
        DbQueryScan localCount =
            new DbQueryScan("SELECT COUNT(*) FROM " + relation.toString(dbms), Schema.ofFields(
                "tupleCount", Type.LONG_TYPE));
        List<Expression> expressions =
            ImmutableList.of(
                new Expression(schema.getColumnName(0), new ConstantExpression(relation
                    .getUserName())),
                new Expression(schema.getColumnName(1), new ConstantExpression(relation
                    .getProgramName())),
                new Expression(schema.getColumnName(2), new ConstantExpression(relation
                    .getRelationName())), new Expression(schema.getColumnName(3),
