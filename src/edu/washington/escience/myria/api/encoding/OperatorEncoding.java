...
/**
 * A JSON-able wrapper for the expected wire message for an operator. To add a
 * new operator, two things need to be done.
 * 
 * 1. Create an Encoding class that extends OperatorEncoding.
 * 
 * 2. Add the operator to the list of (alphabetically sorted) JsonSubTypes
 * below.
 */
...
@JsonSubTypes({
		...
		@Type(name = "ApplyEStep", value = ApplyEStepEncoding.class),
		@Type(name = "ApplyOnes", value = ApplyOnesEncoding.class),
		...
		@Type(name = "EStep", value = EStepEncoding.class),
		@Type(name = "EStepAggregate", value = EStepAggregateEncoding.class),
		...
		@Type(name = "MStepAggregate", value = MStepAggregateEncoding.class),
...
public abstract class OperatorEncoding<T extends Operator> extends
		MyriaApiEncoding {
	 ...
	 */
	public abstract void connect(Operator operator,
	 ...
	 */
	public abstract T construct(@Nonnull ConstructArgs args)
