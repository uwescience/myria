...
/**
 * This class determines what HTTP response will be sent when a web request causes a certain
 * exception type. The standard ones are 500 (INTERNAL SERVER ERROR) and 400 (BAD REQUEST).
 * 
 * Note that any resource wishing to control its response type directly can simply throw a
 * {@link MyriaApiException}, which is a subclass of {@link WebApplicationException} and thus
 * already contains an HTTP Response that will be passed through directly.
 * 
 * 
 */
@Provider
public final class MyriaExceptionMapper implements ExceptionMapper<Throwable> {
  ...
  private static final org.slf4j.Logger LOGGER = LoggerFactory
      .getLogger(MyriaExceptionMapper.class);
  ...
  /**
   * Get the HTTP response for this status and cause. If the Status code is 500 (Internal Server
   * Error) then we return the entire stack trace. Otherwise, we just return the message.
   * 
  ...
  public static Response getResponse(final Status status, final Throwable cause) {
    ...
    if (status.equals(Status.INTERNAL_SERVER_ERROR) || cause.getMessage() == null
