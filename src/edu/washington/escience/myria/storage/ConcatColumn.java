...
public class ConcatColumn<T extends Comparable<?>> extends Column<T> {
	...
	/**
	 * Whether this concatenated column is read only and can no longer be
	 * concatenated to.
	 */
	 ...
	 * 
	 * @param type
	 *            type of the columns that this object exposes.
	 */
	...
	public void addColumn(final Column<?> column) {
		Preconditions.checkState(!readOnly,
				"Cannot add more data to a read only concat column");
	...
	/**
	 * Validates the index of the requested row and returns the necessary
	 * information to calculate get the correct row out of it.
	 * 
	 * @param row
	 *            the row of the requested value.
	 * @return the necessary information to calculate get the correct row and
	 *         value out of the concatenated columns.
	 */
	...
	public MyriaMatrix getMyriaMatrix(final int row) {
		Map.Entry<Integer, Column<?>> entry = getColumnEntry(row);
		return entry.getValue().getMyriaMatrix(row - entry.getKey());
	}
}
