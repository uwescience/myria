...
/**
 * Container class for a batch of tuples. The goal is to amortize memory
 * management overhead.
 * 
 */
@ThreadSafe
public class TupleBatch implements ReadableTable, Serializable {
	 ...
	 * 
	 * @param schema
	 *            schema of the tuples in this batch.
	 * @param isEoi
	 *            whether this TupleBatch is an EOI TupleBatch.
	 * */
	 ...
	 * @return a shallow copy of the specified TupleBatch with the new column
	 *         names.
	 */
	public TupleBatch rename(final List<String> columnNames) {
		Schema newSchema = Schema.of(schema.getColumnTypes(),
				Objects.requireNonNull(columnNames, "columnNames"));
		return new TupleBatch(newSchema, columns, numTuples, isEOI);
	}

	/**
	 * Standard immutable TupleBatch constructor. All fields must be populated
	 * before creation and cannot be changed.
	 * 
	 * @param schema
	 *            schema of the tuples in this batch. Must match columns.
	 * @param columns
	 *            contains the column-stored data. Must match schema.
	 * @param numTuples
	 *            the number of tuples in this TupleBatch.
	 */
	public TupleBatch(final Schema schema,
			final List<? extends Column<?>> columns, final int numTuples) {
	 ...
	 *            schema of the tuples in this batch. Must match columns.
	 * @param numTuples
	 *            the number of tuples in this batch. Must match columns.
	 * @param isEOI
	 *            whether this is an EOI TupleBatch.
	 */
	public TupleBatch(final Schema schema,
			final List<? extends Column<?>> columns, final int numTuples,
			final boolean isEOI) {
		this.schema = Objects.requireNonNull(schema, "schema");
		this.columns = ImmutableList.copyOf(Objects.requireNonNull(columns,
				"columns"));
		Preconditions
				.checkArgument(columns.size() == schema.numColumns(),
		...
		for (Column<?> column : columns) {
			Preconditions.checkArgument(numTuples == column.size(),
					"Column %s != %s tuples", column.size(), numTuples);
		}
	...
	/**
	 * put the tuple batch into TBB by smashing it into cells and putting them
	 * one by one.
	 * 
	 * @param tbb
	 *            the TBB buffer.
	...
	/**
	 * Return a new TupleBatch that contains only the filtered rows of the
	 * current dataset. Note that if some of the tuples in this batch are
	 * invalid, we will have to map the indices in the specified filter to the
	 * "real" indices in the tuple.
	 * 
	 * @param filter
	 *            the rows to be retained.
	 * @return a TupleBatch that contains only the filtered rows of the current
	 *         dataset.
	 */
	public final TupleBatch filter(final BitSet filter) {
		...
		/*
		 * Shortcut: the filter is full, so all current tuples are retained.
		 * Just return this.
		 */
	...
	public final long getLong(final int column, final int row) {
		Preconditions.checkArgument(columns.get(column).size() >= numTuples,
				"numTuples %s columnsize %s", numTuples, columns.get(column)
						.size());
	...
	/**
	 * Partition this TB using the partition function. The method is implemented
	 * by shallow copy of TupleBatches.
	 * 
	 * @return an array of TBs. The length of the array is the same as the
	 *         number of partitions. If no tuple presents in a partition, say
	 *         the i'th partition, the i'th element in the result array is null.
	 * @param pf
	 ...
	 * 
	 * Internal implementation of a (non-duplicate-eliminating) PROJECT
	 * statement.
	 * 
	 * @param remainingColumns
	 *            zero-indexed array of columns to retain.
	 * @param resultSchema
	 *            computing a schema every time is usually not necessary
	...
	public final TupleBatch selectColumns(final int[] remainingColumns,
		...
		return new TupleBatch(resultSchema, newColumns.build(), numTuples,
				isEOI);
	}

	/**
	 * @param rows
	 *            a BitSet flagging the rows to be removed.
	...
	/**
	 * Construct a new TupleBatch that equals the current batch with the
	 * specified column appended. The number of valid tuples in this batch must
	 * be the same as the size of the other batch. If this batch is not dense,
	 * then
	 * 
	 * @param columnName
	 ...
	 * @return a new TupleBatch containing the tuples of this column plus the
	 *         tuples of the other.
	 */
	public TupleBatch appendColumn(final String columnName,
			final Column<?> column) {
		Preconditions.checkArgument(numTuples() == column.size(),
				"Cannot append column of size %s to batch of size %s",
				column.size(), numTuples());
		Schema newSchema = Schema.appendColumn(schema, column.getType(),
				columnName);
		List<Column<?>> newColumns = ImmutableList.<Column<?>> builder()
				.addAll(columns).add(column).build();
