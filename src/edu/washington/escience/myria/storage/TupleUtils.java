...
public final class TupleUtils {
   ...
   */
  public static void copyValue(final ReadableColumn from, final int fromRow,
  ...
  /**
   * Compares a whole tuple with a tuple from another batch. The columns from the two compare
   * indexes are compared in order.
   * 
   * @param table1 the first table
   * @param compareIndexes1 the columns from this table that should be compared with the column of
   *        the other table
   * @param rowIdx1 row in this table
   * @param table2 other table
   * @param compareIndexes2 the columns from the other TB that should be compared with the column of
   *        this table
  ...
  /**
   * Same as {@link #tupleCompare(ReadableTable, int[], int, ReadableTable, int[], int, boolean[])}
   * but comparison within the same table.
   * 
   * @param table the table that compared cells is in
   * @param columnCompareIndexes the columns from this TB that should be compared with the column of
   *        the other table
   ...
   *         equal to, or greater than the second
   */
  public static int tupleCompare(final ReadableTable table, final int[] columnCompareIndexes,
      final int rowIdx, final int rowIdx2, final boolean[] ascending) {
    return tupleCompare(table, columnCompareIndexes, rowIdx, table, columnCompareIndexes, rowIdx2,
        ascending);
  }
   ...
   */
  public static boolean tupleEquals(final ReadableTable table1, final int[] compareColumns1,
      final int row1, final ReadableTable table2, final int[] compareColumns2, final int row2) {
    ...
    for (int i = 0; i < compareColumns1.length; ++i) {
      switch (table1.getSchema().getColumnType(compareColumns1[i])) {
        case BOOLEAN_TYPE:
          if (table1.getBoolean(compareColumns1[i], row1) != table2.getBoolean(compareColumns2[i],
              row2)) {
            return false;
          }
          break;
        case DOUBLE_TYPE:
          if (table1.getDouble(compareColumns1[i], row1) != table2.getDouble(compareColumns2[i],
              row2)) {
            return false;
          }
          break;
   ...
   * @return true if equals
   */
  public static boolean tupleEquals(final ReadableTable table1, final int row1,
      final ReadableTable table2, final int row2) {
   ...
   * @return true if equals
   */
  public static boolean tupleEquals(final ReadableTable table1, final int[] compareColumns,
      final int row1, final ReadableTable table2, final int index) {
