...
/**
 * Used for creating TupleBatch objects on the fly. A helper class used in,
 * e.g., the Scatter operator. Currently it doesn't support random access to a
 * specific cell. Use TupleBuffer instead.
 * 
 * 
 */
public class TupleBatchBuffer implements AppendableTable {
	...
	/**
	 * Convenience constant; must match schema.numColumns() and
	 * currentColumns.size().
	 */
	...
	/**
	 * Internal state representing the number of columns that are ready in the
	 * current tuple.
	 */
	private int numColumnsReady;
	/**
	 * Internal state representing the number of tuples in the in-progress
	 * TupleBatch.
	 */
	...
	/**
	 * Constructs an empty TupleBatchBuffer to hold tuples matching the
	 * specified Schema.
	 * 
	 * @param schema
	 *            specified the columns of the emitted TupleBatch objects.
	 */
	...
	public final void appendTB(final TupleBatch tb) {
		/*
		 * If we're currently building a batch, we better finish it before we
		 * append this one to the list. Otherwise reordering will happen.
		 */
		finishBatch();
	...
	/**
	 * Helper function: checks whether the specified column can be inserted
	 * into.
	 * 
	 * @param column
	...
	private void checkPutIndex(final int column) {
		...
		Preconditions
				.checkState(!columnsReady.get(column),
	...
	/**
	 * Helper function to update the internal state after a value has been
	 * inserted into the specified column.
	 * 
	 * @param column
	 *            the column in which the value was put.
	 */
	...
	/**
	 * Makes a batch of any tuples in the buffer and appends it to the internal
	 * list.
	 * 
	...
	private boolean finishBatch() {
		...
		/* Build the batch */
		List<Column<?>> buildingColumns = new ArrayList<Column<?>>(
				currentBuildingColumns.size());
		for (ColumnBuilder<?> cb : currentBuildingColumns) {
			buildingColumns.add(cb.build());
		}
		readyTuples.add(new TupleBatch(schema, buildingColumns,
				currentInProgressTuples));
	 ...
	 * 
	 * @return a List<TupleBatch> containing all complete tuples that have been
	...
	public final List<TupleBatch> getAll() {
		...
		if (currentInProgressTuples > 0) {
			output.add(new TupleBatch(schema, getInProgressColumns(),
					currentInProgressTuples));
		}
		return output;
	}
	...
	/**
	 * Build the in progress columns. The builders' states are untouched. They
	 * can keep building.
	 * 
	...
	public final TupleBatch popAny() {
		...
		if (tb != null) {
			...
			if (currentInProgressTuples > 0) {
				...
				Preconditions.checkState(size == batch.numTuples(),
						"Error with number of tuples");
				Preconditions.checkState(currentInProgressTuples == 0,
						"Error with in progress tuples");
				return batch;
			} else {
				return null;
			}
	...
	public final TupleBatch popAnyUsingTimeout() {
		...
		if (tb != null) {
			...
			if (currentInProgressTuples > 0
					&& getElapsedTime() >= MyriaConstants.PUSHING_TB_TIMEOUT) {
				final int size = currentInProgressTuples;
				finishBatch();
				updateLastPoppedTime();
				readyTuplesNum -= size;
	 ...
	 * 
	 * @return the first complete TupleBatch in this buffer, or null if none is
	 *         ready.
	 */
	...
	/**
	 * Append a complete tuple coming from two tuple batches: left and right.
	 * Used in join operators.
	 * 
	 * @param leftTb
	 *            the left tuple batch
	 * @param leftIdx
	 *            the index of the left tuple in the tuple batch
	 * @param leftAnswerColumns
	 *            an array that specifies which columns from the left tuple
	 *            batch
	 * @param rightTb
	 *            the right tuple batch
	 * @param rightIdx
	 *            the index of the right tuple in the tuple batch
	 * @param rightAnswerColumns
	 *            an array that specifies which columns from the right tuple
	 *            batch
	 * 
	 */
	public final void put(final TupleBatch leftTb, final int leftIdx,
			final int[] leftAnswerColumns, final TupleBatch rightTb,
			final int rightIdx, final int[] rightAnswerColumns) {
		for (int i = 0; i < leftAnswerColumns.length; ++i) {
			TupleUtils.copyValue(
					leftTb.getDataColumns().get(leftAnswerColumns[i]), leftIdx,
	...
	/**
	 * Append the specified value to the specified destination column in this
	 * TupleBatchBuffer from the source column.
	 * 
	 * @param destColumn
	 *            which column in this TBB the value will be inserted.
	 * @param sourceColumn
	 *            the column from which data will be retrieved.
	 * @param sourceRow
	 *            the row in the source column from which data will be
	 *            retrieved.
	 */
	public final void put(final int destColumn,
			final ReadableColumn sourceColumn, final int sourceRow) {
		TupleUtils.copyValue(sourceColumn, sourceRow, this, destColumn);
	}

	/**
	 * Append the referenced row from the source {@link TupleBatch} to this
	 * {@link TupleBatchBuffer}.
	 * 
	 * @param sourceBatch
	 *            the {@link TupleBatch} from which data will be retrieved.
	 * @param sourceRow
	 *            the row in the source column from which data will be
	 *            retrieved.
	 */
	...
	public void putObject(final int column, final Object value) {
		checkPutIndex(column);
		currentBuildingColumns.get(column).appendObject(
				MyriaUtils.ensureObjectIsValidType(value));
		columnPut(column);
	}
	...
	/**
	 * Add the specified {@link TupleBatch} to this buffer. The implementation
	 * is O(1) when possible, i.e. if the TupleBatch is full and this buffer is
	 * not building a partially-complete TupleBatch. Otherwise, it's O(N) in the
	 * size of the TupleBatch because it is a full copy.
	 * 
	 * @param tupleBatch
	 *            the tuple data to be added to this buffer.
	 */
