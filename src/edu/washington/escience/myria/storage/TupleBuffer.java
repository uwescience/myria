...
/**
 * A simple collection of tuples that provides random access to the inner tuples
 * and can be appended to. Once the user is done appending, they may fetch the
 * {@link #finalResult} of the {@link #TupleBuffer}, which finalizes it and they
 * may access its built contents as a list of {@link TupleBatch}. After
 * finalizing, the {@link TupleBuffer} can no longer have values appended to it.
 */
public class TupleBuffer implements ReadableTable, AppendableTable {
	...
	public TupleBuffer(final Schema schema) {
		...
		Preconditions.checkArgument(numColumns > 0,
				"cannot create a buffer with no columns");
	...
	/**
	 * Actually finish the batch. Does not ensure that the batch is full, and
	 * thus can only be used when finalizing.
	 */
	private void finishBatchEvenIfSmall() {
		...
		Preconditions.checkState(!finalized,
				"cannot force finish a batch once finalized");
	...
	public final MyriaMatrix getMyriaMatrix(final int column, final int row) {
		...
		int localRow = row % TupleBatch.BATCH_SIZE;
		if (batchIndex < readyBatches.size()) {
			return readyBatches.get(batchIndex)
					.getMyriaMatrix(column, localRow);
		}
	...
	public final void putObject(final int column, final Object value) {
		checkPutIndex(column);
		currentBatch.get(column).appendObject(
	...
	private void checkPutIndex(final int column) {
		Preconditions.checkState(!finalized,
				"cannot append to a TupleBuffer once finalized");
