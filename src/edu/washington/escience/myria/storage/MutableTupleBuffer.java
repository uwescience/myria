...
/**
 * A simplified TupleBatchBuffer which supports random access. Designed for hash
 * tables to use.
 */

public class MutableTupleBuffer implements ReadableTable, AppendableTable,
		Cloneable {
	...
	/**
	 * Constructs an empty TupleBuffer to hold tuples matching the specified
	 * Schema.
	 * 
	 * @param schema
	...
	public MutableTupleBuffer(final Schema schema) {
		...
		currentBuildingColumns = ColumnFactory.allocateColumns(schema).toArray(
				new ColumnBuilder<?>[] {});
	...
	private void finishBatch() {
		...
		Preconditions
				.checkArgument(currentInProgressTuples == TupleBatch.BATCH_SIZE);
	...
	public final int numTuples() {
		return readyTuples.size() * TupleBatch.BATCH_SIZE
	...
	public final Object getObject(final int colIndex, final int rowIndex)
		...
		if (tupleBatchIndex > readyTuples.size()
				|| tupleBatchIndex == readyTuples.size()
				&& tupleIndex >= currentInProgressTuples) {
			throw new IndexOutOfBoundsException();
		}
		if (tupleBatchIndex < readyTuples.size()) {
			return readyTuples.get(tupleBatchIndex)[colIndex]
					.getObject(tupleIndex);
		}
	...
	public final MyriaMatrix getMyriaMatrix(final int column, final int row) {
		...
		if (tupleBatchIndex < readyTuples.size()) {
			return readyTuples.get(tupleBatchIndex)[column]
					.getMyriaMatrix(tupleIndex);
		}
		return currentBuildingColumns[column].getMyriaMatrix(tupleIndex);
	}
	...
	public final DateTime getDateTime(final int column, final int row) {
		...
		if (tupleBatchIndex < readyTuples.size()) {
			return readyTuples.get(tupleBatchIndex)[column]
					.getDateTime(tupleIndex);
		}
		return ((DateTimeColumnBuilder) (currentBuildingColumns[column]))
				.getDateTime(tupleIndex);
	}
	...
	public ReadableColumn[] getColumns(final int row) {
		...
		} else if (tupleBatchIndex == readyTuples.size()
				&& tupleIndex < currentInProgressTuples) {
	...
	public final void putObject(final int column, final Object value) {
		checkPutIndex(column);
		currentBuildingColumns[column].appendObject(MyriaUtils
	...
	public final void putMyriaMatrix(final int column, final MyriaMatrix value) {
		checkPutIndex(column);
		currentBuildingColumns[column].appendMyriaMatrix(value);
		columnPut(column);
	}
	 ...
	 * 
	 * @param destColumn
	 *            which column in this TB the value will be inserted.
	 * @param sourceColumn
	 ...
	 *            retrieved.
	 */
	public final void put(final int destColumn, final Column<?> sourceColumn,
			final int sourceRow) {
		TupleUtils.copyValue(sourceColumn, sourceRow, this, destColumn);
	}

	/**
	 * Swap the specified values from sourceRow to destRow in this TupleBuffer
	 * from the given column.
	 * 
	 * @param column
	 *            which column in this TB the value will be inserted.
	 * @param destRow
	 *            the row in the dest column from which data will be retrieved.
	 * @param sourceRow
	 *            the row in the source column from which data will be
	 *            retrieved.
	 */
	public final void swap(final int column, final int destRow,
			final int sourceRow) {
		...
		case MYRIAMATRIX_TYPE:
			MyriaMatrix matrix1 = sourceColumn.getMyriaMatrix(sourceBatchRow);
			MyriaMatrix matrix2 = destColumn.getMyriaMatrix(destBatchRow);
			sourceColumn.replaceMyriaMatrix(matrix2, sourceBatchRow);
			destColumn.replaceMyriaMatrix(matrix1, destBatchRow);
			break;
		case DATETIME_TYPE:
	...
	/**
	 * Replace the specified value to the specified destination column in this
	 * TupleBuffer from the source column.
	 * 
	 * @param destColumn
	 *            which column in this TB the value will be inserted.
	 * @param destRow
	 ...
	 *            retrieved.
	 */
	public final void replace(final int destColumn, final int destRow,
			final Column<?> sourceColumn, final int sourceRow) {
		...
		case BOOLEAN_TYPE:
			...
			break;
		case DATETIME_TYPE:
			dest.replaceDateTime(sourceColumn.getDateTime(sourceRow),
					tupleIndex);
			break;
		case DOUBLE_TYPE:
		...
		case STRING_TYPE:
			...
			break;
		case MYRIAMATRIX_TYPE:
			dest.replaceMyriaMatrix(sourceColumn.getMyriaMatrix(sourceRow),
					tupleIndex);
			break;
		}
	}
	...
	public MutableTupleBuffer clone() {
		...
		for (int i = 0; i < currentBuildingColumns.length; ++i) {
			ret.currentBuildingColumns[i] = currentBuildingColumns[i]
					.forkNewBuilder();
		}
		return ret;
	}
	...
	public ReadableColumn asColumn(final int column) {
		return new ReadableSubColumn(this, Preconditions.checkElementIndex(
				column, numColumns));
	}
