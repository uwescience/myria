...
public class ConditionalExpression extends NAryExpression {
   ...
   */
  public ConditionalExpression(final ExpressionOperator condition,
      final ExpressionOperator firstChoice, final ExpressionOperator secondChoice) {
  ...
  public Type getOutputType(final ExpressionOperatorParameter parameters) {
    ...
    Preconditions.checkArgument(type == Type.BOOLEAN_TYPE,
        "%s requires the first child [%s] to be a boolean expression but it is a %s", getClass()
            .getSimpleName(), getChild(1), type);
    ...
    Preconditions.checkArgument(firstIdx != -1, "%s cannot handle first choice [%s] of Type %s",
        getClass().getSimpleName(), getChild(1), firstType);
    Preconditions.checkArgument(secondIdx != -1, "%s cannot handle second choice [%s] of Type %s",
        getClass().getSimpleName(), getChild(2), secondType);
    return validTypes.get(Math.min(firstIdx, secondIdx));
  }

  @Override
  public String getJavaString(final ExpressionOperatorParameter parameters) {
    return new StringBuilder("(").append(getChild(0).getJavaString(parameters)).append("?")
        .append(getChild(1).getJavaString(parameters)).append(":")
        .append(getChild(2).getJavaString(parameters)).append(")").toString();
  }
}
