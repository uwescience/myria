...
public interface ScriptEvalInterface {
  /**
   * The interface for applying expressions. We only need a reference to the tuple batch and a row
   * id. The variables will be fetched from the tuple buffer using the rowId provided in
   ...
   * @param state state that is passed during evaluation, and written after the new state is
   *        computed.
   */
  void evaluate(final ReadableTable tb, final int rowId, final AppendableTable result,
      final Tuple state);
}
