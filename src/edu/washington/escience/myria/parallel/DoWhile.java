...
public final class DoWhile extends QueryPlan {
  ...
  /**
   * Construct a {@link QueryPlan} that runs the body tasks in sequence, and loops if the singleton
   * boolean relation specified by {@code condition} is {@code true}.
   * 
   * @param body the tasks to be run.
   * @param condition the relation to be scanned for the while loop continuation condition. Must be
   *        a singleton boolean relation.
   */
   ...
   * 
   * 1. Every {@link DoWhile} tracks whether or not the loop has been entered in {@link #hasRun}.
   * For all times but the first, the {@link #condition} is checked and, if {@code false}, the
   * {@link DoWhile} is simply removed from the {@code planQ} and nothing is added. This signifies
   * the loop termination.
   * 
   * 2. When we want to execute the loop, we want to do two things: run the body, then gather the
   * value of the termination condition and store it at the {@link Server} in a query global
   * variable. Thus we want to add the sequence of operations {@code body, then [gatherCondition]}
   * to the plan.
   * 
   * @param planQ the queue of {@link QueryPlan} tasks
   * @param subQueryQ the queue of {@link SubQuery} tasks
   * @param args the {@link QueryConstruct#ConstructArgs} arguments needed to instantiate a query
   *        plan
   */
  @Override
