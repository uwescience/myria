...
/**
 * This class updates the Catalog metadata to reflect datasets that have been created by a query.
 * This metadata updating is triggered when the query finishes.
 * 
 * 
 */
public final class DatasetMetadataUpdater implements OperationFutureListener {
  ...
  /**
   * Create a new DatasetMetadataUpdater, which will update the specified catalog to reflect the
   * creation of the specified datasets, each stored on its specified set of workers, when the query
   * it is listening to finishes successfully.
   * 
  ...
  public DatasetMetadataUpdater(@Nonnull final MasterCatalog catalog,
    ...
    LOGGER.debug("DatasetMetadataUpdater configured for query #{} with relations-worker map {}",
        subQueryId, relationsCreated);
  }
  ...
  public void operationComplete(final OperationFuture future) throws Exception {
    if (!future.isSuccess()) {
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug("SubQuery #{} failed, so not updating the catalog metadata for relations {}.",
            subQueryId, relationsCreated.keySet());
      }
      return;
    }
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("SubQuery #{} succeeded, so updating the catalog metadata for relations {}.",
          subQueryId, relationsCreated.keySet());
    }
