...
public abstract class StreamIOChannel {
  ...
  private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory
      .getLogger(StreamIOChannel.class);
  ...
  final Channel attachIOChannel(final Channel ioChannel) {
    ...
    if (LOGGER.isTraceEnabled()) {
      LOGGER.trace(this.getClass().getSimpleName() + " ID: {} attached to physical channel: {}",
          id, ChannelContext.channelToString(ioChannel), new ThreadStackDump());
    }
    return oldOne;
  }
  ...
  final Channel detachIOChannel() {
    ...
    if (LOGGER.isTraceEnabled()) {
      LOGGER.trace(this.getClass().getSimpleName() + " ID: {} detached from physical channel: {}",
          id, ChannelContext.channelToString(ch), new ThreadStackDump());
    }
    return ch;
  }
