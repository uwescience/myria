 ...
 * 
 * @param <T>
 *            type of the objects in this column.
 * 
 * 
 */
public abstract class Column<T extends Comparable<?>> implements
		ReadableColumn, Serializable {
	...
	/**
	 * Serializes this column as a protobuf message into the specified output
	 * stream.
	 * 
	 ...
	 * 
	 * @param validIndices
	 *            the rows of the column to serialize.
	 * @return a ColumnMessage containing a serialized copy of this column.
	 */
	...
	/**
	 * Creates a new Column containing the contents of this column including
	 * only the specified rows.
	 * 
	 * @param filter
	 *            a BitSet indicating which rows should be kept.
	 * @return a new Column containing the contents of this column including
	 *         only the specified rows.
	 */
	...
	/**
	 * @param type
	 *            the type of the column to be returned.
	...
	public static Column<?> emptyColumn(final Type type) {
		...
		throw new UnsupportedOperationException(
				"Allocating an empty column of type " + type);
	}

	/**
	 * A default implementation to serialize any Boolean column to a proto. Full
	 * copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	 * @return a ColumnMessage with a BooleanColumn member.
	 */
	protected static final ColumnMessage defaultBooleanProto(
			final Column<?> column) {
		...
		/*
		 * Note that we do *not* build the inner class. We pass its builder
		 * instead.
		 */
		final BooleanColumnMessage.Builder inner = BooleanColumnMessage
				.newBuilder().setData(bytes.toByteString());
		return ColumnMessage.newBuilder().setType(ColumnMessage.Type.BOOLEAN)
				.setBooleanColumn(inner).build();
	}

	/**
	 * A default implementation to serialize any DateTime column to a proto.
	 * Full copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	...
	protected static ColumnMessage defaultDateTimeProto(final Column<?> column) {
		ByteBuffer dataBytes = ByteBuffer.allocate(column.size() * Long.SIZE
				/ Byte.SIZE);
		for (int i = 0; i < column.size(); i++) {
			dataBytes.putLong(column.getDateTime(i).getMillis());
		}
		dataBytes.flip();
		final DateTimeColumnMessage.Builder inner = DateTimeColumnMessage
				.newBuilder().setData(ByteString.copyFrom(dataBytes));
		return ColumnMessage.newBuilder().setType(ColumnMessage.Type.DATETIME)
				.setDateColumn(inner).build();
	}

	/**
	 * A default implementation to serialize any Double column to a proto. Full
	 * copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	...
	protected static ColumnMessage defaultDoubleProto(final Column<?> column) {
		ByteBuffer dataBytes = ByteBuffer.allocate(column.size() * Double.SIZE
				/ Byte.SIZE);
		for (int i = 0; i < column.size(); i++) {
			dataBytes.putDouble(column.getDouble(i));
		}
		dataBytes.flip();
		final DoubleColumnMessage.Builder inner = DoubleColumnMessage
				.newBuilder().setData(ByteString.copyFrom(dataBytes));
		return ColumnMessage.newBuilder().setType(ColumnMessage.Type.DOUBLE)
				.setDoubleColumn(inner).build();
	}

	/**
	 * A default implementation to serialize any Float column to a proto. Full
	 * copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	...
	protected static ColumnMessage defaultFloatProto(final Column<?> column) {
		ByteBuffer dataBytes = ByteBuffer.allocate(column.size() * Float.SIZE
				/ Byte.SIZE);
		for (int i = 0; i < column.size(); i++) {
			dataBytes.putFloat(column.getFloat(i));
		}
		dataBytes.flip();
		final FloatColumnMessage.Builder inner = FloatColumnMessage
				.newBuilder().setData(ByteString.copyFrom(dataBytes));
		return ColumnMessage.newBuilder().setType(ColumnMessage.Type.FLOAT)
				.setFloatColumn(inner).build();
	}

	/**
	 * A default implementation to serialize any Integer column to a proto. Full
	 * copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	...
	protected static ColumnMessage defaultIntProto(final Column<?> column) {
		ByteBuffer dataBytes = ByteBuffer.allocate(column.size() * Integer.SIZE
				/ Byte.SIZE);
		for (int i = 0; i < column.size(); i++) {
			dataBytes.putInt(column.getInt(i));
		}
		dataBytes.flip();
		final IntColumnMessage.Builder inner = IntColumnMessage.newBuilder()
				.setData(ByteString.copyFrom(dataBytes));
		return ColumnMessage.newBuilder().setType(ColumnMessage.Type.INT)
				.setIntColumn(inner).build();
	}

	/**
	 * A default implementation to serialize any Long column to a proto. Full
	 * copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	...
	protected static ColumnMessage defaultLongProto(final Column<?> column) {
		...
		dataBytes.flip();
		final LongColumnMessage.Builder inner = LongColumnMessage.newBuilder()
				.setData(ByteString.copyFrom(dataBytes));
		return ColumnMessage.newBuilder().setType(ColumnMessage.Type.LONG)
	...
	protected static ColumnMessage defaultProto(final Column<?> column) {
		...
		throw new UnsupportedOperationException("Serializing a column of type "
				+ column.getType());
	}

	/**
	 * A default implementation to serialize any filtered column to a proto.
	 * Full copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	 * @param validIndices
	 *            the valid indices in the column.
	 * @return a ColumnMessage with an appropriate member.
	 */
	protected static ColumnMessage defaultProto(final Column<?> column,
	...
	/**
	 * A default implementation to serialize any String column to a proto. Full
	 * copy.
	 * 
	 * @param column
	 *            the column to be serialized.
	...
	protected static ColumnMessage defaultStringProto(final Column<?> column) {
		final StringColumnMessage.Builder inner = StringColumnMessage
				.newBuilder();
		...
		return ColumnMessage.newBuilder().setType(ColumnMessage.Type.STRING)
