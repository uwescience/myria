 ...
 */
class FilteredMutableColumn<T extends Comparable<?>> extends MutableColumn<T> {
  ...
  /** The inner column. */
  private final MutableColumn<T> inner;
   ...
   */
  protected FilteredMutableColumn(final MutableColumn<T> inner, final int[] indices) {
    this.inner = inner;
   ...
   */
  protected FilteredMutableColumn(final MutableColumn<T> inner, final BitSet filter) {
    this.inner = inner;
  ...
  @Override
  public void replace(final int index, final T value) {
    inner.replace(convertRow(index), value);
  }
