...
/**
 * Provides the abstraction of a full Column filtered to an indicated set of
 * rows.
 * 
 * @param <T>
 *            the type of the inner Column.
 */
class FilteredColumn<T extends Comparable<?>> extends Column<T> {
	...
	/**
	 * Creates a filtered version of the indicated rows in the specified inner
	 * column.
	 * 
	 * @param inner
	 *            the {@link Column} to be filtered.
	 * @param indices
	 *            the rows of the inner Column to be kept.
	 */
	 ...
	 *            the {@link Column} to be filtered.
	 * @param filter
	 *            a BitSet indicating the rows of the inner Column to be kept.
	 */
	...
	public MyriaMatrix getMyriaMatrix(final int row) {
		return inner.getMyriaMatrix(convertRow(row));
	}
