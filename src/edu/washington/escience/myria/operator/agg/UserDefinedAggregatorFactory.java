...
public class UserDefinedAggregatorFactory implements AggregatorFactory {
  ...
  private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory
      .getLogger(UserDefinedAggregatorFactory.class);
  ...
  /**
   * Construct a new user-defined aggregate. The initializers set the initial state of the
   * aggregate; the updaters update this state for every new tuple. The emitters produce the final
   * value of the aggregate. Note that there must be the same number of initializers and updaters,
   * but there may be any number > 0 of emitters.
   * 
   * @param initializers Expressions that initialize the state variables.
   * @param updaters Expressions that update the state variables as a function of the input and the
   *        current tuple.
  ...
  public Aggregator get(final Schema inputSchema) throws DbException {
    if (state == null) {
      ...
      for (int i = 0; i < initializers.size(); i++) {
        Preconditions.checkArgument(Objects.equals(initializers.get(i).getOutputName(), updaters
            .get(i).getOutputName()),
            "initializers[i] and updaters[i] have different names (%s) != (%s)", initializers
                .get(i).getOutputName(), updaters.get(i).getOutputName());
      }
  ...
  /**
   * Produce a {@link ScriptEvalInterface} from {@link Expression}s and
   * {@link ExpressionOperatorParameter}s. This function produces the code for a Java script that
   * executes all expressions in turn and appends the calculated values to the result. The values to
   * be output are calculated completely before they are stored to the output, thus it is safe to
   ...
   * @param param the inputs that expressions may use, including the {@link Schema} of the
   *        expression inputs and worker-local variables.
  ...
  private ScriptEvalInterface getEvalScript(@Nonnull final List<Expression> expressions,
    ...
    for (int varCount = 0; varCount < expressions.size(); ++varCount) {
      ...
      output.append(Expression.RESULT).append(".put")
          .append(type.toJavaObjectType().getSimpleName()).append("(").append(varCount)
    ...
    try {
      return (ScriptEvalInterface) se.createFastEvaluator(script, ScriptEvalInterface.class,
