...
/**
 * The Aggregation operator that computes an aggregate (e.g., sum, avg, max, min). This variant
 * supports aggregates over multiple columns, group by multiple columns.
 * 
 * @see Aggregate
...
public final class MultiGroupByAggregate extends UnaryOperator {
  ...
  public MultiGroupByAggregate(@Nullable final Operator child, final int[] gfields,
    ...
    Preconditions.checkArgument(gfields.length > 1,
        "to use MultiGroupByAggregate, must group over multiple fields");
    Preconditions.checkArgument(factories.length != 0,
        "to use MultiGroupByAggregate, must specify some aggregates");
  ...
  /**
   * Since row <code>row</code> in {@link TupleBatch} <code>tb</code> does not appear in
   * {@link #groupKeys}, create a new group for it.
   * 
   * @param tb the source {@link TupleBatch}
   * @param row the row in <code>tb</code> that contains the new group
   * @param hashMatches the list of all rows in the output {@link TupleBuffer}s that match this
   *        hash.
   * @throws DbException if there is an error.
   */
  private void newGroup(final TupleBatch tb, final int row, final IntArrayList hashMatches)
      throws DbException {
    ...
    Preconditions.checkState(groupKeys.numTuples() == aggStates.size(),
        "groupKeys %s != groupAggs %s", groupKeys.numTuples(), aggStates.size());
  }

  /**
   * Called when there is no list yet of which output aggregators match the specified hash. Creates
   * a new int list to store these matches, and insert it into the {@link #groupKeyMap}.
   * 
   * @param groupHash the hash of the grouping columns in a tuple
   * @return the new (empty still) int list storing which output aggregators match the specified
   *         hash
   */
  ...
  private TupleBatch getResultBatch() throws DbException {
    ...
    Preconditions.checkState(curGroupKeys.numTuples() == aggResults.numTuples(),
        "curGroupKeys size %s != aggResults size %s", curGroupKeys.numTuples(),
    ...
    return new TupleBatch(getSchema(), ImmutableList.<Column<?>>builder()
        .addAll(curGroupKeys.getDataColumns()).addAll(aggResults.getDataColumns()).build());
  }

  /**
   * The schema of the aggregate output. Grouping fields first and then aggregate fields. The
   * aggregate
   * 
