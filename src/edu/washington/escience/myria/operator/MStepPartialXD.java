...
public final class MStepPartialXD extends BinaryOperator {
   ...
   */
  public MStepPartialXD(final Operator left, final Operator right, final int[] compareIndx1,
   ...
   *        types.
   */
  public MStepPartialXD(final List<String> outputColumns, final Operator left, final Operator right,
  ...
  public MStepPartialXD(final List<String> outputColumns, final Operator left, final Operator right,
    ...
    this(outputColumns, left, right, compareIndx1, compareIndx2,
        range(left.getSchema().numColumns()), range(right.getSchema().numColumns()));
  }
  ...
  protected Schema generateSchema() {
    ...
    for (int i = 0; i < 1 + 1 + numDimensions + numDimensions * numDimensions; i++) {
   ...
   */
  protected void addToAns(final TupleBatch cntTB, final int row, final MutableTupleBuffer hashTable,
      final int index) {
  ...
  protected TupleBatch fetchNextReady() throws DbException {
    ...
    if (isEOIReady()) {
      /*
       * New JoinMStep logic: Left child has reached EOS, so we now can output partial
       * responsibilities. ans.putLong(0, 0); nexttb = ans.popAny();
       */
      if (!hasOutputAll) {
   ...
   * 
   * The left child is the points relation, so for each point we will add it to the partial state of
  ...
  protected void processLeftChildTB(final TupleBatch tb) {
    ...
    for (int row = 0; row < tb.numTuples(); ++row) {
      ...
      MyriaMatrix xMatrix = inputColumns.get(indexCounter).getMyriaMatrix(row);
      indexCounter++;

      xArray = xMatrix.getArray();

      double[] responsibilities = new double[numComponents];
      MyriaMatrix respMatrix = inputColumns.get(indexCounter).getMyriaMatrix(row);
      indexCounter++;
      ...
      for (int i = 0; i < numComponents; i++) {
        MyriaMatrix b_j = inputColumns.get(indexCounter).getMyriaMatrix(row);
        bs.add(b_j);
        indexCounter++;
      }
      ...
      // Get the Matrices for B
      for (int i = 0; i < numComponents; i++) {
        MyriaMatrix bSigma_j = inputColumns.get(indexCounter).getMyriaMatrix(row);
        Bs.add(bSigma_j);
        indexCounter++;
      }
      ...
      for (int i = 0; i < numComponents; i++) {
        states[i].addPointXD(bs.get(i).getArrayCopy(), responsibilities[i],
            Bs.get(i).getArrayCopy());
      }
    }
  }
  ...
  private void addToHashTable(final TupleBatch tb, final int row,
      final MutableTupleBuffer hashTable, final TIntObjectMap<TIntList> hashTable1IndicesLocal,
      final int hashCode) {
  ...
  private class PartialState implements Serializable {
    ...
    /**
     * The partial sum of means over the points encountered so far. This is stored in the jblas
     * DoubleMatrix class.
     */
    private DoubleMatrix mu_k_partial;

    /**
     * The partial sum of covariances over the points encountered so far. This is stored in the
     * jblas DoubleMatrix class.
     */
    private DoubleMatrix sigma_k_partial;

    /**
     * The partial sum of means over the points encountered so far. This is stored in the JAMA
     * Matrix class.
     */
    private Matrix jama_mu_k_partial;

    /**
     * The partial sum of covariances over the points encountered so far. This is stored in the JAMA
     * Matrix class.
     */
    ...
    /**
     * A boolean flag indicating whether the final parameters have been computed or not.
     */
    ...
    private void addPointXD(final double[][] bArray, final double r_ik, final double[][] BArray) {
      nPoints += 1;
      ...
      if (matrixLibrary.equals("jblas")) {
        ...
        jama_sigma_k_partial = jama_sigma_k_partial.plus(b_diff.times(r_ik));
      } else {
        throw new RuntimeException("Incorrect matrix libary specified.");
      }
