...
public class LeapFrogJoin extends NAryOperator {
  ...
  private boolean leapfrogSearch() {
    ...
    CellPointer maxKey = new CellPointer(columnToProceed.table, columnToProceed.column, iterators[columnToProceed.table]
    ...
    while (true) {
      ...
      CellPointer leastKey = new CellPointer(fieldWithLeastKey.table, fieldWithLeastKey.column,
      ...
      if (cellCompare(leastKey, maxKey) == 0) { // if the value current
        ...
        if (atEnd) {
          ...
          maxKey = new CellPointer(fieldWithLeastKey.table, fieldWithLeastKey.column, iterators[fieldWithLeastKey.table]
              .getRowOfCurrentField());
          nextIterator();
        }
      }
  ...
  private int cellCompare(final CellPointer cp1, final CellPointer cp2) {
    return TupleUtils.cellCompare(tables[cp1.tableIndex], cp1.getFieldIndex(), cp1.getRow(), tables[cp2
