...
/**
 * JsonTupleWriter is a {@link TupleWriter} that serializes tuples to JavaScript Object Notation
 * (JSON). The output is a list of objects, each of which has one field 'attribute' : 'value' per
 * column.
 * 
 * For a dataset that has two integer columns names 'x' and 'y', and three rows, the output will
 * look like
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Attribute names (column names) are {@link String} objects that are escaped for JSON. For values,
 * primitive types are output unquoted and as-is; {@link DateTime} objects are quoted and serialized
 * in ISO8601 format, and {@link String} objects are quoted and escaped for JSON.
 * 
 * 
 */
public class JsonTupleWriter implements TupleWriter {
  ...
  public void writeTuples(final ReadableTable tuples) throws IOException {
    ...
    for (int i = 0; i < tuples.numTuples(); ++i) {
      ...
      /*
       * Write each column of the record. Primitive types can be output as-is; {@link DateTime}
       * objects are quoted and serialized in ISO8601 format, and {@link String} objects are quoted
       * and escaped for JSON.
       */
  ...
  public void error() throws IOException {
    try {
      ...
      output
          .write("\"error\":\"There was an error. Investigate the query status to see the message\"}]");
      output.flush();
    } finally {
      output.close();
    }
