 ...
 * a {@link QueryFuture} of an I/O operation that supports progress notification, the listener's
 * {@link #operationProgressed(QueryFuture, long, long, long)} method will be called back by an I/O thread. If the
 * operation does not support progress notification, {@link #operationProgressed(QueryFuture, long, long, long)} will
 * not be invoked. Like a usual {@link QueryFutureListener} that this interface extends,
 * {@link #operationComplete(QueryFuture)} will be called when the future is marked as complete.
 * 
 * <h3>Return the control to the caller quickly</h3>
 * 
 * {@link #operationProgressed(QueryFuture, long, long, long)} and {@link #operationComplete(QueryFuture)} is directly
 * called by an I/O thread. Therefore, performing a time consuming task or a blocking operation in the handler method
