...
public class IterativeFailureTest extends SystemTestBase {
  ...
  public TupleBatchBuffer getAJoinResult(
      TupleBatchBuffer startWith, TupleBatchBuffer multiWith, Schema schema) {
  ...
  public void generateJoinPlan(
    ...
    if (isHead) {
      ...
      final GenericShuffleProducer sp1 =
          new GenericShuffleProducer(
              scan1, joinArrayID, new int[] {workerIDs[0], workerIDs[1]}, pf1);
      sc1 =
          new GenericShuffleConsumer(
              tableSchema, joinArrayID, new int[] {workerIDs[0], workerIDs[1]});
      workerPlan.get(0).add(sp1);
      workerPlan.get(1).add(sp1);
    } else {
      sc1 =
          new GenericShuffleConsumer(
              tableSchema, receivingOpID, new int[] {workerIDs[0], workerIDs[1]});
    }
    ...
    if (sendingOpID != null) {
      ...
      final GenericShuffleProducer sp_others_worker1 =
          new GenericShuffleProducer(
              send2others_worker1, sendingOpID, new int[] {workerIDs[0], workerIDs[1]}, pf1);
      final GenericShuffleProducer sp_others_worker2 =
          new GenericShuffleProducer(
              send2others_worker2, sendingOpID, new int[] {workerIDs[0], workerIDs[1]}, pf1);
  ...
  public void joinChainWorkerPartitionFailureTest() throws Throwable {
    ...
    for (RootOperator worker0Root : workerPlan.get(0)) {
      ...
      final SingleRandomFailureInjector srfi =
          new SingleRandomFailureInjector(0, TimeUnit.SECONDS, 1.0, di);
  ...
  public TupleBatchBuffer getCircularJoinResult(
      TupleBatchBuffer a, TupleBatchBuffer b, TupleBatchBuffer c, Schema schema) {
