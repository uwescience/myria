...
public class MasterDaemonTest {
  ...
  public void testStartAndRestShutdown() throws Exception {
    ...
    try {
      ...
      Invocation.Builder shutdownInvocation =
          client.target("http://localhost:" + REST_PORT + "/server/shutdown").request(
      ...
      shutdownInvocation.property(HttpAuthenticationFeature.HTTP_AUTHENTICATION_BASIC_USERNAME,
          "jwang").property(HttpAuthenticationFeature.HTTP_AUTHENTICATION_BASIC_PASSWORD,
          "somepassword");
      ...
      shutdownInvocation.property(HttpAuthenticationFeature.HTTP_AUTHENTICATION_BASIC_USERNAME,
          "admin").property(HttpAuthenticationFeature.HTTP_AUTHENTICATION_BASIC_PASSWORD,
          "wrongpassword");
      assertEquals(Status.UNAUTHORIZED.getStatusCode(), shutdownInvocation.get().getStatus());

      /* Provide the correct admin password and stop the master. */
      shutdownInvocation.property(HttpAuthenticationFeature.HTTP_AUTHENTICATION_BASIC_PASSWORD,
          "password");
  ...
  public void testNoAdminPasswordCfgFile() throws Exception {
    ...
    Invocation.Builder invocation =
        client.target("http://localhost:" + REST_PORT + "/server/deployment_cfg").request(
            MediaType.TEXT_PLAIN_TYPE);
